...
void InitializeAllBPBs(VOID)
{
  ...
  int drive, fileno;
#if defined(NEC98)
  for (drive = 'A'; drive < 'A' + LoL->nblkdev; drive++)
#else
...
#if defined(NEC98)
      { 0x29, FP_OFF(int29_handler) },
#endif
      { 0x2a, FP_OFF(int2a_handler) },
      { 0x2f, FP_OFF(int2f_handler) }
#if defined(NEC98)
      , { 0xdc, FP_OFF(intdc_handler) }
#endif
    };
  struct vec *pvec;
  int i;
...
#if defined(NEC98)
#define InitPrinters_nec98  InitPrinters
STATIC VOID InitPrinters_nec98(VOID)
{
}
#elif defined(IBMPC)
STATIC VOID InitPrinters(VOID)
...
#else
#error need platform specific InitPrinters()
#endif

#if defined(NEC98)
#define InitSerialPorts_nec98  InitSerialPorts
STATIC VOID InitSerialPorts_nec98(VOID)
{
}
#elif defined(IBMPC)
STATIC VOID InitSerialPorts(VOID)
...
#else
#error need platform specific InitSerialPorts()
#endif

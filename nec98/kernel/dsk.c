...
{
#if defined(NEC98)
  UNREFERENCED_PARAMETER(pddt); /* todo */
#else
...
#else
#error missing platform specific play_dj() 
#endif
...
STATIC int LBA_to_CHS(ULONG LBA_address, struct CHS *chs, const ddt * pddt)
{
  ...
  if (LBA_address > LBA_CYLINDER_MAX)
  {
#ifdef DEBUG
    printf("LBA-Transfer error : cylinder %lu > %u\n", LBA_address, (unsigned)LBA_CYLINDER_MAX);
#else

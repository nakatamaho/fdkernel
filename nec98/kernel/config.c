...
# define VK_F8 0x6900
# define VK_UP 0x3A00
# define VK_DOWN 0x3D00
#else
#error missing platform specific VK_F5, VK_F8, VK_UP and VK_DOWN.
#endif
...
STATIC void WriteMenuLine(struct MenuSelector *menu)
{
  ...
  printf("%s", pText);
#else
#error platform specific WriteMenuLine...
#endif
}
...
STATIC int SkipLine_first(int check_shift);
#if 0
...
#if defined(JAPAN)
  nlsPackageHardcoded.cntry = 81;
  nlsPackageHardcoded.cp = 932;
#endif
}
...
extern unsigned char  cdecl __inportb__(int);
#define my_inp(portid)  __inportb__(portid)
...
{
  if (attr < 0 || attr > 0x1f) attr = 7;
  printf("\x1b[%dm", 30 + (attr & 7));
}

#define GetBiosKey(t)  GetBiosKey_nec98(t,0)
#define GetBiosKey2(t,s)  GetBiosKey_nec98(t,s)
#elif defined(IBMPC)
...
#define GetBiosKey2(t,s) GetBiosKey(t)
#else
...
STATIC BOOL SkipLine(char *pLine)
{
  ...
  for (;;)
  {
    ...
    if (key == VK_F5)          /* YES, you may hit F5 here, too */
    {
      printf("N\n");
      SkipAllConfig = TRUE;
      return TRUE;
...
*/
#if defined(NEC98)
#define sysScreenMode_nec98 sysScreenMode
STATIC VOID sysScreenMode_nec98(BYTE * pLine)
{
...
#else
#error missing platform specific sysScreenMode()
#endif
    ...
    if (*pLine == '/') {
      ...
      case 'K':
        ...
        if (commands[0].pass == 1)
          kbdType = 0; /* force conv keyb (IBMPC) */
#endif
        break;
      case 'N':
        InitKernelConfig.SkipConfigSeconds = -1;
...
*/
#if defined(NEC98)
#define CfgKeyBuf_nec98 CfgKeyBuf
STATIC VOID CfgKeyBuf_nec98(BYTE * pLine)
{
  UNREFERENCED_PARAMETER(pLine);
}
#elif defined(IBMPC)
...
#else
#error missing platform specific CfgKeyBuf()
#endif
...
#if defined(NEC98)
#define Numlock_nec98 Numlock
STATIC VOID Numlock_nec98(BYTE * pLine)
{
  UNREFERENCED_PARAMETER(pLine);
}
#elif defined(IBMPC)
...
#else
#error missing platform specific Numlock()
#endif
...
#if defined(NEC98)
      printf("\x1b[%d;4H", nMenuLine + 1); /* set cursor pos */
#elif defined(IBMPC)
      r.a.b.h = 0x02;
      r.b.b.h = 0;
      r.d.b.l = 3;
...
#else
#error missing platform specific "set cursor pos" in DoMenu() 
#endif
    }
    ...
    else if(key == VK_UP && MenuColor != -1)      /* arrow up */
    {
    ...
    else if(key == VK_DOWN && MenuColor != -1)      /* arrow down */
    {
...
#if defined(NEC98)
#define ClearScreen_nec98 ClearScreen
STATIC void ClearScreen_nec98(unsigned char attr)
{
  /* clear and move cursor to pos(0,0): */
  printf("\x1b*");
  /* set color: */
  SetTextColor_nec98(attr);
  MenuColor = attr;
}
#elif defined(IBMPC)
STATIC void ClearScreen(unsigned char attr)
...
#else
#error missing platform specific ClearScreen() 
#endif
...
#if 1 || defined(JAPAN)
{ 81,_DATE_YMD,"\x81\x8f",',','.', '/',':', 0 , 2,_TIME_24},/* Japan         */
#else
...
#if defined(NEC98)
  installMemory = ((unsigned)_init_end + 15) / 16;
#else
